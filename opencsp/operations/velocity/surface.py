# opencsp/operations/velocity/surface.py
import random
import numpy as np
...
def initialize_surface_velocity(structure: Any) -> np.ndarray:
    """
    初始化表面结构的速度
    ...
    # 为每个原子生成随机速度
    # 表面结构：z方向速度较小，xy方向速度较大
    velocity = np.random.normal(0.0, 0.1, (num_atoms, 3))
    velocity[:, 2] *= 0.3  # 减小z方向速度
    
    return velocity

class SurfaceVelocityUpdate(StructureOperation):
    """适用于表面的速度更新操作"""
    ...
    def apply(self, current_velocity: np.ndarray, particle: Individual, 
        ...
        if dim != 2:
            raise ValueError(f"SurfaceVelocityUpdate只适用于表面(2D)，但接收到的维度为{dim}")
